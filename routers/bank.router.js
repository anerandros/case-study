...
router.post("/save", async (req, res) => {
    ...
    try {
        ...
        await MySQLManager.addToTable(MySQLManager.tables.BANK, parsedBody);
        res.send({
            status: "OK",
...
router.get("/:bankId", async (req, res) => {
    ...
    try {
        ...
        // N.B.: MAI inviare dati in DB senza parsarli. Si omette per semplicità.
        const result = await MySQLManager.readTableWhere(MySQLManager.tables.BANK, "id_bank", bankId);
        res.send({
            status: "OK",
            result,
        });
...
router.get("/", async (req, res) => {
    try {
        // N.B.: MAI inviare dati in DB senza parsarli. Si omette per semplicità.
        const results = await MySQLManager.readTable(MySQLManager.tables.BANK);
        res.send({
            status: "OK",
            results,
        });
